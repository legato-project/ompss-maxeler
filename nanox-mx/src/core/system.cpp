...
#ifdef MAX_DEV
   verbose0( "loading Max DFE support" );
   if ( !loadPlugin( "pe-max" ) )
      fatal0 ( "couldn'l load Max DFE support" );
#endif
...
global_reg_t System::_registerMemoryChunk(void *addr, std::size_t len)
{
   ...
   global_reg_t reg;
   getHostMemory().getRegion( cd, reg, NULL );
   return reg;
}
...
void System::_distributeObject( global_reg_t &reg, unsigned int start_node, std::size_t num_nodes )
{
   ...
   for ( std::size_t node_idx = 0; node_idx < num_nodes; node_idx += 1 ) {
      ...
      getHostMemory().getRegion( cd, fragmented_reg, NULL );
